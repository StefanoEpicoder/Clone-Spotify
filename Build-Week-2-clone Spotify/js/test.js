...
let arrayDiId = [
	81763, 420041687, 6364781, 69319552, 1238967, 521266992, 128938202,
	401889417, 350417267, 508204251,
];
let playerPlaylist = [];
let songIndex = -1;
let isPlaying = false;
...
const createCard = (obj, cardType) => {
	if (cardType == "small") {
		...
		dFlexContainer.className =
			"d-flex justify-content-center align-item-center position-relative";

		const imgAlbumCover = document.createElement("img");
		imgAlbumCover.src = obj.album.cover;
		imgAlbumCover.className =
			"card-img-top max-h-180 object-fit-cover mx-2 mt-2 rounded";
		imgAlbumCover.alt = "...";
...
async function heroFetch(random) {
	try {
		const response = await fetch(
			"https://deezerdevs-deezer.p.rapidapi.com/album/" + random,
			{
				method: "GET",
				headers: {
					"X-RapidAPI-Host": "deezerdevs-deezer.p.rapidapi.com",
